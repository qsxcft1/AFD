...
def get_norm(norm_layer, dim):
    if norm_layer == nn.LayerNorm:
        return nn.LayerNorm(dim)
    elif norm_layer == nn.BatchNorm2d:
        return nn.BatchNorm2d(dim)
    else:
        raise NotImplementedError(f"Normalization layer {norm_layer} is not supported.")
...
class SwinEncoder(nn.Module):
    ...
    def load_from(self, pretrained_path):
        ...
        if pretrained_path is not None:
            ...
            def _resize_rel_pos_bias(src_bias, dst_shape):
                if src_bias.dim() != 2 or len(dst_shape) != 2:
                    return None
                if src_bias.shape[1] != dst_shape[1]:
                    return None

                src_len, num_heads = src_bias.shape
                dst_len, _ = dst_shape

                src_size = int(math.sqrt(src_len))
                dst_size = int(math.sqrt(dst_len))
                if src_size * src_size != src_len or dst_size * dst_size != dst_len:
                    return None

                src = src_bias.view(src_size, src_size, num_heads).permute(2, 0, 1).unsqueeze(0)
                src = F.interpolate(src, size=(dst_size, dst_size), mode='bicubic', align_corners=False)
                src = src.squeeze(0).permute(1, 2, 0).contiguous().view(dst_len, num_heads)
                return src
            ...
            for k in list(full_dict.keys()):
                ...
                if full_dict[k].shape == model_dict[k].shape:
                    loaded_dict[k] = full_dict[k]
                    found += 1
                    continue

                if "relative_position_bias_table" in k:
                    resized = _resize_rel_pos_bias(full_dict[k], model_dict[k].shape)
                    if resized is not None:
                        loaded_dict[k] = resized
                        found += 1
                        interpolated += 1
                    else:
                ...
                if k == "patch_embed.proj.weight":
                    src = full_dict[k]
                    dst_shape = model_dict[k].shape
                    if src.dim() == 4 and len(dst_shape) == 4 and src.shape[0] == dst_shape[0] and src.shape[2:] == dst_shape[2:] and dst_shape[1] == 4 and src.shape[1] == 3:
                        new_w = model_dict[k].clone()
                        new_w[:, :3, :, :] = src
                        new_w[:, 3:4, :, :] = src.mean(dim=1, keepdim=True)
                        loaded_dict[k] = new_w
                        found += 1
                    else:
                        print("delete:{};shape pretrain:{};shape model:{}".format(k, src.shape, dst_shape))
                    continue
            ...
            print(f"Encoder Interpolated RelPosBias: {interpolated}")
            print(f"Encoder Skipped Buffers: {skipped}")
        else:
            print("none pretrain")
